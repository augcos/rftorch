...
class Continuous_PPO_Agent():
    def __init__(self, actor_network, critic_network, action_limit=1, gamma=0.99, gae=0.95, 
                    clipping=0.2, n_epochs=10, batch_size=64):
        self.actor_network = actor_network
        self.critic_network = critic_network
    ...
    def get_action(self, state):
        ...
        action_sampled = action_probs.sample()
        prob = action_probs.log_prob(action_sampled).item()
        action = T.tanh(action_sampled) * self.action_limit
    ...
    def learn(self):
        for _ in range(self.n_epochs):
            ...
            for batch in batch_indices:
                ...
                self.actor_network.eval()
                self.critic_network.eval()
                mu, std = self.actor_network.forward(state_batch)
                actor_probs = T.distributions.Normal(mu, T.exp(std))
